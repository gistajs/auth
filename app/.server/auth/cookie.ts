...
export async function requireUserFromCookie(request: Request) {
...
export async function requireLoggedOut(request: Request) {
  let public_id = await getSessionValue(request, USER_ID_KEY)
  if (public_id) throw await redirectWithWarning('/', 'Already logged in')
}
...
export async function login(
  ...
  return redirectWithSuccess(path, toast, { headers })
}
...
function safeRedirect(to: unknown, defaultRedirect = '/') {
