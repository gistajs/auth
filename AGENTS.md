...
- For related pages under one route group, prefer flat route files (for example, `app.settings.tsx`, `app.notes.$id.tsx`) instead of nested folders
...
- `requireUserFromCookie(request)` for protected routes
- `getUserFromCookie(request)` for optional auth in loaders
