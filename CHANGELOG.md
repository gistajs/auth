...
Made starter TypeScript configuration compatible with TypeScript 7

- Removed the obsolete baseUrl option while preserving relative import alias mappings.

Replaced Arctic with portable Google and GitHub OAuth clients

- Included small Google and GitHub OAuth clients that can be configured with your own credentials.
- Added GitHub S256 PKCE and require a verified primary email while retaining the user:email scope.
- Google sign-in now checks ID token issuer, audience, authorized party, expiration, and verified email directly after code exchange.
- OAuth callbacks now connect the provider to the signed-in account instead of switching accounts; connecting an account requires a matching verified email.
- Reject replacement of existing Google or GitHub account IDs during sign-in and linking; preserve safe redirects and display actionable link errors.
- Removed the deprecated Arctic dependency.

Breaking change.

Upgrade notes:
Keep environment configuration in routes/oauth/+/google.ts and github.ts and migrate those adapters to createGoogle/createGitHub. Apply the OAuth routes and shared helpers together with app/.server/oauth. Existing cookie and identity JSON shapes are unchanged. GitHub users need a verified primary email, and in-flight GitHub logins started before PKCE was added must be restarted. Accounts with a conflicting linked provider ID are rejected rather than overwritten.
